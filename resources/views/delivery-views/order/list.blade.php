                            ...
                            <div class="card-body">
                                <div class="flex-between align-items-center mb-1">
                                    <div style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                                        <h6 class="card-subtitle text">{{\App\CPU\translate('pending')}}</h6>
                                    ...
                                    <div class="mt-2">
                                        <i class="tio-shopping-cart" style="font-size: 50px !important; color: #EB7F25;  font-size:50px !important;"></i>
                                    </div>
                                </div>
                                <!-- End Row -->
                            </div>
                    ...
                    <div class="col-lg-6 col-md-6 col-sm-12 mb-3 mb-lg-5">
                        ...
                        <a class="card card-hover-shadow h-100" href="{{route('admin-delivery.orders.list',['confirmed'])}}" style="background: #FFFFFF;">
                            <div class="card-body">
                                <div class="flex-between align-items-center mb-1">
                                    <div style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                                        <h6 class="card-subtitle text">{{\App\CPU\translate('confirmed')}}</h6>
                                        <span class="card-title h2">
                                            {{\App\Model\Order::when(session()->has('show_inhouse_orders') && session('show_inhouse_orders') == 1,function($q){
                                                $q->whereHas('details', function ($query) {
                                                    $query->whereHas('product', function ($query) {
                                    ...
                                    <div class="mt-2">
                                        <i class="tio-checkmark-circle" style="font-size: 50px !important; color: #EB7F25;  font-size:50px !important;"></i>
                                    </div>
                                </div>
                                <!-- End Row -->
                            </div>
                        ...
                        <a class="card card-hover-shadow h-100" href="{{route('admin-delivery.orders.list',['processing'])}}" style="background: #FFFFFF">
                            <div class="card-body">
                                <div class="flex-between align-items-center gx-2 mb-1">
                                    <div style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                                        <h6 class="card-subtitle text">{{\App\CPU\translate('Processing')}}</h6>
                                        <span class="card-title h2">
                                            {{\App\Model\Order::when(session()->has('show_inhouse_orders') && session('show_inhouse_orders') == 1,function($q){
                                                $q->whereHas('details', function ($query) {
                                                    $query->whereHas('product', function ($query) {
                                    ...
                                    <div class="mt-2">
                                        <i class="tio-time" style="font-size: 50px !important; color: #EB7F25;  font-size:50px !important;"></i>
                                    </div>
                                </div>
                                <!-- End Row -->
                            </div>
                    ...
                    <div class="col-lg-6 col-md-6 col-sm-12 mb-3 mb-lg-5">
                        ...
                        <a class="card card-hover-shadow h-100" href="{{route('admin-delivery.orders.list',['out_for_delivery'])}}" style="background: #FFFFFFff">
                            <div class="card-body">
                                <div class="flex-between align-items-center gx-2 mb-1">
                                    <div style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                                        <h6 class="card-subtitle text">{{\App\CPU\translate('out_for_delivery')}}</h6>
                                        <span class="card-title h2">
                                            {{\App\Model\Order::when(session()->has('show_inhouse_orders') && session('show_inhouse_orders') == 1,function($q){
                                                $q->whereHas('details', function ($query) {
                                                    $query->whereHas('product', function ($query) {
                                    ...
                                    <div class="mt-2">
                                        <i class="tio-bike" style="font-size: 50px !important; color: #EB7F25;  font-size:50px !important;"></i>
                                    </div>
                                </div>
                                <!-- End Row -->
                            </div>
                        ...
                        <div class="row gx-lg-4">
                            <div class="col-lg-7 col-md-6 col-sm-12">
                                <div class="card-body flex-between align-items-center class" style="cursor: pointer"
                                    ...
                                    <div>
                                        <span class="icon icon-sm icon-soft-secondary icon-circle ml-3">
                                            <i class="tio-checkmark-circle-outlined" style="color: #EB7F25;  font-size:50px !important;"></i>
                                        </span>
                                    </div>
                                </div>
                                <div class="d-lg-none">
                            ...
                            <div class="col-lg-5 col-md-6 col-sm-12">
                                <div class="card-body flex-between align-items-center class" style="cursor: pointer"
                                    ...
                                    <div class="media-body" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                                        <h6 class="card-subtitle text">{{\App\CPU\translate('unpaid')}}</h6>
                                        <span
                                            class="card-title h3">
                                            {{\App\Model\Order::when(session()->has('show_inhouse_orders') && session('show_inhouse_orders') == 1,function($q){
                                                $q->whereHas('details', function ($query) {
                                    ...
                                    <span class="icon icon-sm icon-soft-secondary icon-circle ml-3">
                                    <i class="tio-message-failed" style="color: #EB7F25;  font-size:50px !important;"></i>
                                    </span>
                                </div>
                                <div class="d-lg-none">
                                    <hr>
                    ...
                    <div class="col-12 col-md-5 mt-2 mt-sm-0">
                        <form action="{{ url()->current() }}" id="form-data" method="GET">
                            ...
                            <div class="row">
                                ...
                                <div class="col-12 col-sm-2 mt-2 mt-sm-0  ">
                                    <button type="submit" class="btn btn-success float-right float-sm-none" onclick="formUrlChange(this)" data-action="{{ route('admin-delivery.orders.order-bulk-export', ['status' => $status]) }}">
                ...
                <table class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table"
                    ...
                    @foreach($orders as $key=>$order)
                        ...
                        <tr class="status-{{$order['order_status']}} class-all">
                            ...
                            <td >
                                <a href="{{route('admin-delivery.orders.details',['id'=>$order['id']])}}">{{$order['id']}}</a>
                            </td>
                            <td>{{date('d M Y',strtotime($order['created_at']))}}</td>
                            <td>
                                @if($order->customer)
                                    <a class="text-body text-capitalize"
                                       href="{{route('admin-delivery.orders.details',['id'=>$order['id']])}}">{{$order->customer['f_name'].' '.$order->customer['l_name']}}</a>
                                @else
                            ...
                            <td>
                                ...
                                <a class="btn btn-primary btn-sm mr-1" title="{{\App\CPU\translate('view')}}"
                                    href="{{route('admin-delivery.orders.details',['id'=>$order['id']])}}"><i
                                        class="tio-visible"></i> </a>
                                <a class="btn btn-info btn-sm mr-1" target="_blank" title="{{\App\CPU\translate('invoice')}}"
                                    href="{{route('admin-delivery.orders.generate-invoice',[$order['id']])}}"><i
...
@push('script_2')
    <script>
        function filter_order() {
            $.get({
                url: '{{route('admin-delivery.orders.inhouse-order-filter')}}',
                contentType: false,
                processData: false,
                beforeSend: function () {
                    $('#loading').show();
